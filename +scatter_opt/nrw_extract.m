function out = nrw_extract(Sdata,L,L_air,lambda_c,varargin)
...
%     % Check if principal branch is valid (generally not useful)
%     % Based on Arslanagic et al. 2013 condition for electrically thin
%     % slabs (eq. 28). pb_valid value of 1 does NOT necessarily imply that 
%     % the principal branch is the correct branch, just that it could be the
%     % correct branch. pb_valid value of 0 indicates that the principal 
%     % branch is not the correct branch.
%     if basebranch==0
%         ks = nrw_ks(data.s11,data.s21,L,basebranch);
%         prod = abs(real(ks)).*L;
%         if max(prod) < pi
%             pb_valid = 1;
%         else
...
%         
%         if pb_valid==0
%             warning('Principal branch is not valid')
%         end
%     end
...
% Manual branch increment replaced by phase unwrapping log(1/tc) in nrw_lambda    
%     % check for discontinuities
...
%                 % for each step, increment the branch by 1
%                 idx = step_index(j);
%                 branch(idx+1:end) = branch(idx+1:end) + 1;
%             end
%             % recalculate lambda
%             lam = nrw_lambda(tc,L,branch);
%             % recalculate mu and eps
%             mu = (1+rc)./(lam.*(1-rc).*sqrt((1./lambda_0.^2)-(1./lambda_c.^2)));
%             eps = (lambda_0.^2./mu).*((1./lambda_c.^2)+1./lam.^2);
%         end
%         
...
%             step_index = find(abs(ediff) > 20*mean(abs(ediff)));
%             if ~isempty(step_index)
%                 warning(['Found remaining discontinuity in epsilon after incrementing branch. Interpolating points. Step indexes:',mat2str(step_index)])
